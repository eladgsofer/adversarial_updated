...
def execute():
    ...
    h_adv_jamming_uniform = noise_scalar * original_h

    print("BeamForming Rate (attacked via adding traditional noise with ratio) "
          "rate: {0} ratio: {1}".format(classical_model.objec(h=h_adv_jamming_uniform, wa=wa_original,
                                                              wd=wd_original).mean().item(),
                                        (h_adv_jamming_uniform - original_h).norm(2) / original_h.norm(2)))

    #   print("BeamForming Rate (attacked via adding traditional noise with ratio) "
    #         "rate: {0} ratio: {1}".format(classical_model.objec(h=noise_scalar * original_h, wa=wa_original,
    #                                                             wd=wd_original).mean().item(),
    #                                       (noise_scalar * original_h).norm(2)/(original_h.norm(2))))
    ...
    for h_idx in range(H_test.shape[1]):
        ...
        max_values.append(abs(original_h).max().item())

        print(h_idx)
        for e_idx, eps in enumerate([0.15]):
            bf_model = ProjGA(mu)

            adv_h, _, wa_hat, wd_hat = beamforming_BIM(bf_model, original_h, eps=eps)

            attacked_rate = classical_model.objec(h=original_h, wa=wa_hat, wd=wd_hat).norm(2).item()

            rates[h_idx, e_idx] = attacked_rate

            min_ratios.append(((adv_h - original_h).norm(2)).item() / original_h.norm(2).item())


    min_ratios = np.array(min_ratios)
    # mid_ratios = np.array(mid_ratios)
    # max_ratios = np.array(max_ratios)
    # np.save(min_ratios, "min_ratios.npy")
    # np.save(max_ratios, "max_ratios.npy")
    print("epsilon 0.15:")
    print(min_ratios.mean())
    # print("epsilon 0.15: {0}")
    # print(mid_ratios.mean())
    # print("epsilon 0.2: {0}")
    # print(max_ratios.mean())
